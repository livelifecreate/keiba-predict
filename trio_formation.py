...
相手幅の決定根拠（2026-09-22 総当たり検証・案Dの順位・テスト2025/07〜 n=886R・払戻ベース）:
  1軸-相手2〜5位(6点)   的中20.1% / ROI85%  ← 採用（上位3件除外73%・2025後半90%・2026年81%）
  1軸-相手2〜6位(10点)  的中25.2% / ROI72%  ← 旧設定。的中率は上がるがROIは落ちる
  1軸-相手2〜7位(15点)  的中30.2% / ROI65%
  軸を予想2位にすると的中率が半減（2〜6位で8.9%）→ 軸は予想1位で確定。
  ※ROIは85%でも100%未満。買いサインは引き続き記録のみ（実際の購入判断はユーザー）。

（参考・旧）決定根拠（全期間 n=688 バックテスト・払戻ベース）:
