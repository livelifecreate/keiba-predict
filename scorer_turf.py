...
# -------------------------------------------------------------------
# スコア項目 係数（グリッドサーチ最良値 2026-06-17）
# 変更する場合は A案→B案 の数値に書き換えるだけ
#   A案（現行）: 訓練1-4月 900組み合わせ探索の最良値
#   B案（保守）: 統計的に安全な控えめ値
# -------------------------------------------------------------------
_COEFF_WIN    = 2.50   # A案: 2.50 / B案: 1.75
_COEFF_PREV   = 0.25   # A案: 0.25 / B案: 0.50
_COEFF_RISING = 1.25   # A案: 1.25 / B案: 1.25（共通）
...
def check_jockey_bonus(jockey: str, is_grade: bool) -> float:
    """重賞限定騎手ボーナス"""
    if not is_grade:
        return 0.0
    for name, bonus in JOCKEY_BONUS_GRADE.items():
        if name in jockey:
            return bonus
    return 0.0
...
def check_content_score(recent: list[PastRace]) -> float:
    ...
    ① 着順: 1着+3 / 2着+2 / 3着+1.5 / 4-5着+0.5  ← 基準、係数なし
    ② 着差: ×0.5係数（着順と重複するため過大にならないよう調整）
    ④ 上がり3F: ×0.5係数（毎回積み重なりやすいため調整）
    ⑤ 脚質×位置取り: 係数なし（条件付き加点なので過大になりにくい）
    ⑥ 人気乖離: ×0.5係数（穴馬ボーナスが過大に積み重なるため調整）
    キャップ撤廃 → 自然な分布で上位馬の差がつくようにする
    """
    if not recent:
        return 0.0
    ...
    for i, p in enumerate(recent[:3]):
        ...
        if pos > 1 and 0 <= p.margin <= 0.5:
            if p.margin <= 0.0:    s += 2.0 * 0.5
            elif p.margin <= 0.1:  s += 1.5 * 0.5
            elif p.margin <= 0.3:  s += 1.0 * 0.5
            else:                   s += 0.5 * 0.5
...
def score_all(entries: list, race_info, training_data: dict = None,
              ...
              weight_diffs: dict = None, senko_list: list = None,
              horse_baba_history: dict = None) -> list[tuple]:
    """
    entries             : HorseEntry のリスト
    race_info           : RaceInfo
    training_data       : {馬名: TrainingData} netkeiba から取得済みのデータ（省略可）
    horse_baba_history  : {馬名: [(馬場状態, 着順), ...]} verify_batch から渡す（省略可）
    """
    ...
    horse_baba_history = horse_baba_history or {}

    # 道悪: pre-fetchデータ優先、なければ馬IDから取得
    track_cond_cache: dict[str, list[tuple[str, int]]] = {}
    if track_condition and track_condition != "良":
        for entry in entries:
            if entry.horse_name in horse_baba_history:
                track_cond_cache[entry.horse_name] = horse_baba_history[entry.horse_name]
            else:
    ...
    for i, entry in enumerate(entries):
        ...
        d = ScoreBreakdown(
            ...
            rising_trend           = check_rising_trend(recent) * _COEFF_RISING,
...
def save_csv(results: list[tuple], race_info, odds_map: dict = None, training_data: dict = None, sign_tag: str = None, eval_comment: list = None, race_id: str = "", sign_level: str = None, sign_detail_text: str = None, race_class: int = 0, track_condition: str = ""):
    ...
    with open(filepath, "w", newline="", encoding="utf-8-sig") as f:
        ...
        writer.writerow([])
        writer.writerow(["■レース情報", surface_label, race_id, track_condition or ""])
...
def print_buy_signs(sorted_results: list[tuple], race_info, odds_map: dict = None):
    """買いサイン・見送りサインを表示（参考用・実運用はweekend_predict.py）"""
    if len(sorted_results) < 2:
        return
    ...
    skips = []
    if n == 18:
        skips.append(f"18頭フルゲート")
    if 3 <= gap < 5:
        skips.append(f"1〜2位スコア差{gap:.1f}pt（3〜5ptゾーン）")

    # ── 状況メモ ─────────────────────────────────────────────
    notes = []
    if gap < 1:
        notes.append(f"横並び乖離{gap:.1f}pt")
    if 14 <= n <= 17:
        notes.append(f"{n}頭立て")
    ...
    if skips:
        ...
        buy = "三連複4頭BOX (4点)" if getattr(race_info, "race_class", 0) == 3 else "三連複5頭BOX (10点)"
        print(f"  買い目: {buy}")
        print(f"     ┗ 対象: 1位({top_entry.horse_name})〜上位{'4' if '4頭' in buy else '5'}頭BOX")
        for note in notes:
            print(f"  ✅ {note}")


def print_scores(results: list[tuple], race_info, odds_map: dict = None):
    odds_map     = odds_map or {}
    ...
    # 買い判断サイン（オッズを馬名キーで渡す）
    name_odds = {entry.horse_name: odds_map.get(entry.horse_number, 0)
                 for entry, _ in sorted_results}
    print_buy_signs(sorted_results, race_info, name_odds)
...
if __name__ == "__main__":
    ...
    for source, key in race_list:
        ...
        # オッズ自動取得（--oddsが未指定の時）
        race_id_for_odds = direct_race_id or (race_id if race_keyword else None)
        if not odds_map and race_id_for_odds:
            from netkeiba_race_scraper import fetch_odds
            print("  [オッズ] netkeibaから自動取得中...")
            odds_map = fetch_odds(race_id_for_odds)
            if odds_map:
                print(f"  [オッズ] {len(odds_map)}頭分を取得")
            else:
                print("  [オッズ] 取得できませんでした（発走前のみ有効）")
        ...
        if odds_map:
            ...
            if not race_id_for_odds:  # 手動入力の場合のみ表示
                print(f"  [オッズ] {len(horse_odds)}頭分を反映")
        print_scores(results, race_info, odds_map=odds_map if odds_map else None)
