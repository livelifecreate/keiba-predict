...
class ScoreBreakdown:
    ...
    class_achievement:     float = 0.0  # 0〜+4（格×着差のクラス実績・class_achievement.py。ON時はgrade系を置換）
    # 手動チェック用（自動採点には含めない）
...
def score_all(entries: list, race_info, training_data: dict = None,
    ...
    if track_condition and track_condition != "良":
        ...
        for entry in entries:
            ...
            if hid:
                track_cond_cache[entry.horse_name] = (past_track_conditions_cached(hid, getattr(race_info, 'date', ''))
                                                      or fetch_past_track_conditions(hid))
            else:
...
def save_csv(results: list[tuple], race_info, odds_map: dict = None, training_data: dict = None, sign_tag: str = None, eval_comment: list = None, race_id: str = "", sign_level: str = None, sign_detail_text: str = None, race_class: int = 0, track_condition: str = "", presorted: bool = False, extra_cols: dict = None, extra_sections: list = None):
    ...
    with open(filepath, "w", newline="", encoding="utf-8-sig") as f:
        ...
        if len(sorted_results) >= 2:
            ...
            # クラス別に 4box/5box と分けていたが、予想1位が最も信頼できる軸であることが
            # 判明し、1軸-相手型に統一した。詳細は trio_formation.py 参照。
